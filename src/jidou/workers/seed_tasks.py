    ...
    try:
        async with session_factory() as session:
            task = await create_task_record(
                session, celery_task_id, "seed", progress_total=0, dry_run=dry_run
            )
            if task.status in {
                ...
                logger.info(
                    "Seed task %s already %s; skipping redelivery",
                    celery_task_id,
                    task.status,
                )
            ...
            await update_task_status(
                ...
                celery_task_id,
                TaskStatus.RUNNING,
                progress_message="Listing remote files…",
            )
            ...
            completed = await update_task_status(
                ...
                progress_message=f"Seed complete: {result.files_seeded} files seeded",
        ...
        logger.exception("Seed task failed")
        async with session_factory() as session:
            await update_task_status(
                session,
                celery_task_id,
