...
from jidou.services.progress import check_task_cancelled, mark_task_timed_out, update_task_status
...
_SHOW_HANDLED_COLUMNS = frozenset(
    {
        ...
        "episode_groups",
        "episode_group_map",
        "active_episode_group_id",
        "active_episode_group_name",
        "status",
        "in_production",
        "number_of_seasons",
        "number_of_episodes",
...
async def _db_import(celery_task_id: str, file_content: str) -> str:
    ...
    async def _work(
        ...
        for row in shows_data:
            ...
            current += 1
            await on_progress(current, total, f"Shows: {tmdb_id}")
        ...
        for row in episodes_data:
            ...
            actual_show_id = show_id_map.get(backup_show_id) if backup_show_id is not None else None
            ...
            current += 1
            if current % 100 == 0:
                await on_progress(current, total, f"Episodes: {current}")
    ...
    return await run_task_workflow(
        ...
        progress_total=0,
        dry_run=False,
        running_message="Parsing backup file…",
    )
...
def _build_show(row: dict[str, Any]) -> Show:
    ...
    return Show(
        ...
        active_episode_group_id=row.get("active_episode_group_id"),
        active_episode_group_name=row.get("active_episode_group_name"),
...
def _update_show(show: Show, row: dict[str, Any]) -> None:
    ...
    show.active_episode_group_id = row.get("active_episode_group_id", show.active_episode_group_id)
    show.active_episode_group_name = row.get(
        "active_episode_group_name", show.active_episode_group_name
    )
    ...
    show.banner_path = row.get("banner_path", show.banner_path)
