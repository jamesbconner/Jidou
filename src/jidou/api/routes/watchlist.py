...
async def _ensure_rss_stub(session: AsyncSession, show_id: int, show_title: str) -> None:
    ...
    Resolution order:
    1. An existing subscription already linked to this show_id — do nothing.
    2. An unlinked subscription (show_id IS NULL) whose name matches show_title
       case-insensitively or with token-set ratio ≥ 85 — set show_id and return.
    3. No match — create a new inactive stub.

    The savepoint on stub insertion prevents a concurrent-insert IntegrityError
    from rolling back the enclosing watchlist transaction.
...
async def create_watchlist_entry(
    ...
    if existing is not None:
        if "status" in payload.model_fields_set and existing.status != payload.status:
            ...
            await db_session.refresh(existing, ["updated_at"])
            logger.debug(
    ...
    await db_session.refresh(entry, ["show", "updated_at"])
...
async def update_watchlist_entry(
    ...
    await db_session.flush()
    await db_session.refresh(entry, ["updated_at"])
