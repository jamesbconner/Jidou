...
- Split the crowded **General** tab into **General** (read-only env-backed configuration), **Features** (Appearance + Dashboard toggles + Recommendations — everything you visually/functionally customize), **Services** (health checks/connection tests, TMDB cache, sync/RSS schedules), and **Data** (unchanged: import/export, SFTP baseline, metadata backfill) — no persisted-settings changes, purely a layout split to reduce scroll depth on the page users actually adjust most often.
